...
#### Решение
Время создания пакета: 56 * 8 / (128 * 1024) ~ 0.0035 c

Время передачи: 56 * 8 / (1024 * 1024) ~ 0.000448 с

Задержка: 0.0035 + 0.000448 + 0.005 ~ 0.008948 с
...
Средняя общая задержка:  0.01 +  0.01 = 0.02 с

Скорость поступления пакетов: 10 / 0.02 = 500 пакетов / с
...
1) L / $R_S$ + $d_{\text{распространения}}$
2) Да если первый пакет все еще передается через вторую линию. T должен быть дольше времени передачи во второй линии = L / $R_C$
...
1) Delta = 850000 / (15 * 1024 * 1024) = 0.054 с
 
2) B = 16 * Delta = 0.86 о / c

   Delay = 3 + 0.054 / (1 - 0.054 * 0.86) = 3.056
   
3) Cached Delay = 3 + 0.4 * 0.054 / (1 - 0.054 * 0.86) = 3.02
